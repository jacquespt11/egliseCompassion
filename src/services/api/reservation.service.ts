// src/services/api/reservation.service.ts
/**
...
export async function checkAvailability(
  ...
  startTime: string,
  endTime: string
): Promise<{ available: boolean; conflicts?: ApiReservation[] }> {
  return await apiClient.post('/reservations/check-availability', {
    roomId,
    startTime,
    endTime,
  });
 ...
 */
export async function getReservationStats(userId?: number): Promise<any> {
  const params = userId ? { userId } : {};
  return await apiClient.get('/reservations/stats', { params });
}
